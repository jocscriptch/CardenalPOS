...
class Inventarios extends Controller
{
    ...
    {
        if (empty($datos)) {
            $data = $this->model->getMovimientos($this->idUsuario);
        } else {
            $array = explode('-', $datos);
            $anio = $array[0];
            $mes = $array[1];
            $data = $this->model->getMovimientosMes($anio, $mes, $this->idUsuario);
        }
    ...
    public function reporte($datos)
    {
        ...
        if (empty($datos)) {
            $data['inventario'] = $this->model->getMovimientos($this->idUsuario);
        } else {
            $array = explode('-', $datos);
            $anio = $array[0];
            $mes = $array[1];
            $data['inventario'] = $this->model->getMovimientosMes($anio, $mes, $this->idUsuario);
        }
        ob_start();
    ...
    public function procesarAjuste()
    {
        ...
        if (empty($datos['idProducto'])) {
            ...
            $res = array('msg' => 'LA CANTIDAD ES REQUERIDA', 'type' => 'warning');
        } else {
    ...
    {
        $data['empresa'] = $this->model->getEmpresa();
        $data['kardex'] = $this->model->getKardex($idProducto, $this->idUsuario);
